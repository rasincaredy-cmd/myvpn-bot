...
class Settings(BaseSettings):
    ...
    platega_secret: str = ""
    # Порт локального приёмника уведомлений об оплате (наружу его выставляет
    # nginx с TLS-сертификатом — Platega принимает только https с доверенным
    # сертификатом). 0 = приёмник не поднимается, статусы добирает поллинг.
    webhook_port: int = 0
    # Путь к сертификату приёмника — только чтобы задать его вручную, если в
    # /etc/letsencrypt/live окажется несколько папок. Пусто = бот находит сам.
    webhook_cert_path: str = ""

    # ── Мини-приложение (WebApp) ─────────────────────────────────────────────
    # Адрес страницы мини-приложения, например https://31.77.157.162/app/.
    # Пусто = мини-приложения нет: кнопка в меню не показывается, API не
    # поднимается вовсе. Адрес обязан быть https с сертификатом от доверенного
    # центра — Telegram другого не откроет. Раздаёт страницу тот же nginx, что
    # принимает уведомления об оплате (см. scripts/nginx-webhook.conf), поэтому
    # мини-приложению нужен и WEBHOOK_PORT.
    miniapp_url: str = ""
    ...
    # Раз в сколько минут обходить серверы и слать тревоги о поломках
    # (bot/services/health.py). 0 = выключено. 10 минут — компромисс: авария
    # замечается быстро, а лишних входов по SSH банилка не видит.
