...
Компромисс размера: полная инверсия RU — ~21 600 маршрутов (~348 КБ конфиг) —
РОНЯЕТ клиент Amnezia при импорте (весь AllowedIPs — одна строка, которую
приложение целиком тащит в превью/настройки). Разбить на несколько строк
`AllowedIPs =` нельзя: парсер импорта Amnezia складывает ключи в QMap
(importController.cpp, повторный ключ затирает предыдущий) — выживет только
последняя строка, конфиг молча станет дырявым. Поэтому напрямую пускаем
RU-блоки с prefixlen <= RU_DIRECT_MAX_PREFIXLEN (по умолчанию /21 — 92%
адресного пространства RU, все ключевые сервисы: mail.ru и сбер в /21,
vk/яндекс/госуслуги в /17–/18, reg.ru/avito в /20; итого ~8 500 маршрутов,
~132 КБ — ниже проверенно-рабочих 11 216 из amnezia-client#2248). Мелкие
RU-подсети уходят в туннель — это безопасное направление ошибки:
заблокированный сайт никогда не окажется «напрямую» и не отвалится, просто
чуть больше трафика поедет через VPN.

Такой конфиг раздаётся ТОЛЬКО .conf-файлом: столько маршрутов не влезает
ни в QR (~3 КБ максимум), ни в vpn://-ссылку (лимит сообщения 4096).
