...
async def cb_dev_ru(call: CallbackQuery, session: AsyncSession) -> None:
    ...
    await tg_bot.send_message(
        call.message.chat.id,
        "🇷🇺 <b>Режим «RU напрямую»</b>: крупные российские сети (vk, яндекс, "
        "госуслуги, большинство провайдеров) идут мимо VPN — твой обычный IP "
        "и скорость, всё остальное — через VPN.\n\n"
        ...
        "<i>Часть RU-сервисов (например mail.ru, ozon) всё же поедет через "
        "VPN — они сидят в мелких сетях, а полный их список приложение не "
        "тянет. Работать будут, просто через туннель. Встроенное раздельное "
        "туннелирование Amnezia на этом конфиге недоступно — так устроен "
        "клиент; банковские приложения удобнее исключать на обычном конфиге: "
        "Настройки → Разделение туннелирования → по приложениям.</i>",
    )
