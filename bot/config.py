...
class Settings(BaseSettings):
    ...
    # /21 = 92% RU-адресов и все ключевые сервисы (mail.ru и сбер сидят в /21,
    # vk/яндекс/госуслуги в /17–/18, reg.ru/avito в /20): ~8500 маршрутов, ~132КБ.
    # НЕ поднимать до /24–/32 (полное покрытие): ~21600 маршрутов / 348КБ роняют
    # клиент Amnezia при импорте (проверено 19.07.2026); ориентир «сколько клиент
    # переваривает» — 11216 маршрутов из amnezia-client#2248 (Android/iOS ок,
    # Windows подключается минуты). /22 (+ozon/wildberries) — уже 15000, рискованно.
    ru_direct_max_prefixlen: int = 21
