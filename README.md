...
- **Режим «RU напрямую»** — кнопка в карточке устройства выдаёт версию конфига с раздельным туннелированием на уровне WireGuard AllowedIPs: крупные российские подсети (снапшот ipdeny в `bot/assets/ru_networks.txt`, блоки ≤ `/18` — порог `RU_DIRECT_MAX_PREFIXLEN`, ≈ 66% RU-адресов: vk, яндекс, госуслуги, большинство провайдеров) и LAN идут мимо VPN, весь остальной мир — через туннель. Работает в любом WG-клиенте; выдаётся только `.conf`-файлом (~2100 маршрутов, ~31 КБ — не влезают в QR и `vpn://`). Порог выше НЕ поднимать — упирается в клиент Amnezia, эмпирика 19.07.2026 на Android: `/21` (8496 маршрутов) — туннель не стартует, `/32` (21643) — краш при импорте. mail.ru/сбер (`/21`), ozon/wildberries (`/22`) едут через VPN — работают, просто через туннель; это же безопасное направление ошибки: заблокированный ресурс не окажется «напрямую». Сплит по доменам (как `regexp:.*\.ru$` в XRay-клиентах) в WireGuard невозможен — у WG нет SNI-сниффинга, только IP-маршруты. Обычные конфиги — полный туннель со строкой `AllowedIPs = 0.0.0.0/0, ::/0` ровно в том виде, в котором клиент Amnezia распознаёт полный туннель и разблокирует своё меню раздельного туннелирования (на «RU напрямую»-конфиге это меню клиент блокирует — by design Amnezia).
