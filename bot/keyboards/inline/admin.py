...
def admin_user_bypass_card_kb(
    ...
    if is_active:
        ...
        # Отвязка от устройства — та же кнопка, что есть у юзера: жалоба «пишет
        # неверный пароль» приходит в поддержку раньше, чем человек находит её
        # сам, и разговор должен кончаться здесь, а не «попробуйте нажать у себя».
        kb.button(text="📱 Отвязать устройство", callback_data=f"{CB_PANEL}:ubpu:{access_id}:{user_id}:{page}")
