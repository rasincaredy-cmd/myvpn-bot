...
def build_peer_conf(
    *,
    ...
    туннелирования (amnezia-client #2206). Параметр оставлен для гибкости
    (кастомный сплит маршрутами WG работает в любом клиенте), но продуктовой
    фичи на нём нет: «RU напрямую» удалена 19.07.2026 — клиент Amnezia не тянет
    большие AllowedIPs (>~8k маршрутов туннель не стартует, >~21k краш), а
    нужный Владу сплит «банки/ozon напрямую» требует доменного роутинга (XRay),
    в WireGuard его не существует."""
