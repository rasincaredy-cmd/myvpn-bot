...
class Settings(BaseSettings):
    ...
    # /32 = ПОЛНОЕ покрытие RU-реестра (100%, все блоки вплоть до /25–/26):
    # mail.ru, reg.ru и прочие сервисы в мелких блоках идут напрямую, иначе
    # часть рунета всё равно шла бы через VPN. ~21600 маршрутов, файл ~350КБ —
    # не проблема, раздаём .conf-файлом (не QR/vpn://). Снизить (напр. /20 ≈ 87%,
    # ~90КБ) только если на слабом телефоне туннель поднимается дольше (rusplit.py).
    ru_direct_max_prefixlen: int = 32
