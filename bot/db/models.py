...
class User(Base):
    ...
    # Когда юзеру отправили руку помощи «конфиг забрал, но так и не подключился»
    # (планировщик, секция 1c). NULL = ещё не отправляли. Дата, а не флаг: по ней
    # видно, когда именно человек застрял, — а это единственная точка воронки,
    # где юзер уже дошёл до конфига и всё равно не поехал. Шлём ОДИН раз за всю
    # жизнь аккаунта: не подключился и после подсказки — это уже не «не понял»,
    # и вторым сообщением мы бы просто надоедали.
    onboard_help_sent_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True)
    )
    ...
    # На сколько месяцев юзер покупал подписку в последний раз — чтобы
    # автопродление брало тот же срок (и ту же скидку за срок), а не всегда
    # месяц. Пишется ТОЛЬКО при покупке с баланса. NULL = неизвестно: юзер
    # покупал до появления поля или срок выставил админ вручную — таким
    # автопродление даёт месяц, как раньше.
    sub_term_months: Mapped[int | None] = mapped_column(Integer)
...
class AuditLog(Base):
    ...
    #
    # ЗНАК: сумма хранится по модулю, направление денег читается из кода события
    # (BALANCE_TOPUP/REFERRAL_REWARD — юзеру, BALANCE_CHARGE — с юзера). Это НЕ
    # как в BalanceTx, где «плюс пополнение, минус списание». Исключение —
    # ADMIN_CREDIT: админ одним кодом и начисляет, и списывает, поэтому там знак
    # значащий и брать его по модулю нельзя. Лента админки (fmt_audit_row)
    # опирается ровно на эту конвенцию.
